...
Non-motorised transport - Veloland Schweiz (Federal Roads Office FEDRO, cantons)
In this task, you will analyze the cycle network of Switzerland using R. The dataset comes from the [Federal Roads Office FEDRO / ASTRA](https://map.geo.admin.ch/#/map?lang=en&layers=ch.astra.veloland&bgLayer=void). 
...
4. To compute the centrality measures you will need to 
   ...
   2. Use `dplyr::mutate` to create new columns and populate these with the centrality measures using the functions `centrality_betweenness`, `centrality_closeness` and `centrality_degree` from `tidygraph`
...
5. The centrality values are attached to the points. Transfer the values to the lines by using the following function, which takes the mean centrality values over all nodes for each edge
   ...
   ```
5. Visualize the result. If necessary, transform or reasonably categorize the values to make them more comparable
