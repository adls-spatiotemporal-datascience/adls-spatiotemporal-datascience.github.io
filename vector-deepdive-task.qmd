...
:::{.callout-important}
Due date for the tasks are listed in [the schedule](index.qmd#tbl-schedule)
:::

Follow the instructions in [Submission via GitHub Pages](submission.qmd#sec-gh-pages) to create a new repo in the existing organization. Solve the tasks in a file named `index.qmd`.
...
Use this dataset and with the help of [DuckDB in practice](vector-deepdive-duckdb.qmd#sec-duckdb-practice), recreate [Task 1](#sec-vec-basic) and measure the execution time using the R package [`tictoc`](http://jabiru.github.io/tictoc/). 

Compare the execution time with [Task 1](#sec-vec-basic). Discuss!
