...
#| eval: false
library(sf)
luftqualitaet <- st_read("data/interpolation/luftqualitaet.gpkg")
...
Another simple option for interpolation is the nearest neighbour approach, that we can recreate using voronoi polygons. Use the approach described in [Exercise 2: Voronoi](interpolation-density.qmd#sec-density-voronoi) to create voronoi polygons. Turn the resulting `sfc` object to `sf` using `st_as_sf`, then use `st_join` to add the measured NO2 values the polygons. 
